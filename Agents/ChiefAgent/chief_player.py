...
class ChiefPlayer(Player):
    def __init__(
        ...
        pool_ids,
        num_samples=10,
        avoid_knowledge_rollback=False,
    ):
        self.name = name
        self.pnr = pnr
    ...
    def get_action(self, game_state, player_model, action_default=None):
        ...
        self.sequential_partnerknowledgemodels_chief_persp.append(
            self._make_partner_knowledge_model(game_state)
        )
        ...
        if action.type == PLAY or action.type == DISCARD:
            ...
            for cid in self.card_ids:
                if self.card_ids[cid] < action.cnr:
                    ...
                    self.played_or_discarded_card = [
                        cid,
                        [],
                        self.drawn_dict[cid],
                    ]  # will fill in knowledge during inform
    ...
    def inform(self, action, player, game_state, player_model):
        ...
        if self.game_state_before_move == None:  # CHIEF going second this means
            ...
            self.game_state_before_move = GameState(
                self.partner_nr,
                hands,
                [],
                [],
                [(c, 0) for c in ALL_COLORS],
                3,
                game_state.valid_actions,
                8,
                [[initial_knowledge()] * 5] * 2,
            )
            self.player_model_before_move = BasePlayerModel(
                self.pnr, [initial_knowledge()] * 5, [], []
            )
        ...
        #########################################################################
        if (
            (len(changed_cards) > 0) or self.played_or_discarded_card != None
        ) and not self.avoid_knowledge_rollback:
        ...
        modified_game_state.hands = [
            None if a == [] else [] for a in self.game_state_before_move.hands
        ]
        ...
        for a in game_state.valid_actions:
            if a.type in [PLAY, DISCARD] and a.cnr >= len(
                game_state.all_knowledge[self.partner_nr]
            ):
                continue
            else:
                VA.append(a)
        ...
        modified_player_model = BasePlayerModel(
            self.partner_nr,
            self.game_state_before_move.all_knowledge[self.partner_nr],
            self.hints_to_partner,
            self.player_model_before_move.get_actions(),
        )
        ...
        self.move_tracking_table = self.move_tracking_table.append(
            pd.Series(data=new_row, name=self.move_idx)
        ...
        sample_conditionals = [
            sample.conditional_probs
            for sample in self.move_tracking_table.loc[
                self.move_idx, "generated samples"
            ]
        ]
        new_conditional = np.mean(np.array(sample_conditionals), axis=0)

        self.move_tracking_table.at[
            self.move_idx, "conditional probabilities"
        ] = new_conditional
        ...
        if self.move_idx == 0:
            self.move_tracking_table.at[
                self.move_idx, "agent distribution"
            ] = self._makeprob(new_conditional)
            self.move_tracking_table.at[
                self.move_idx, "MLE probabilities"
            ] = new_conditional
        else:
            ...
            self.move_tracking_table.at[
                self.move_idx, "agent distribution"
            ] = self._makeprob(updated_prior)
            self.move_tracking_table.at[self.move_idx, "MLE probabilities"] = (
                new_conditional + prior2 * self.move_idx
            ) / (self.move_idx + 1)
    ...
    def _get_action_helper(self):
        ...
        for i, agent_id in enumerate(agent_ids):
            bc_output = BehaviorClone.sequential_predict(
                agent_id,
                self.sequential_gamestates_chief_persp,
                self.sequential_playermodels_chief_persp,
                self.sequential_partnerknowledgemodels_chief_persp,
    ...
    def _new_sample(self, new_knowledge):
        ...
        for card in new_knowledge:
            card_idx = self._weighted_sample(
                CardChoices, self._makeprob(new_knowledge[card])
    ...
    def _agent_probs(
        self, samp_values, move_idx
    ):  ## THIS ASSUMES THAT WE ONLY HAVE ONE TEAMMATE
        ...
        for idx in range(move_idx + 1):
            game_state_ref, base_player_model_ref = self.move_tracking_table.iloc[idx][
                "observable game state"
            ]
        ...
        for agent_id in agent_ids:
            bc_output = BehaviorClone.sequential_predict(
                ...
                game_states,
                base_player_models,
                [self._make_partner_knowledge_model(gs) for gs in game_states],
            )
            actionvalues = np.zeros(20)
    ...
    def _rollforward_move_tracker(
        self, move_index, most_recent_move, cards_affected_with_new_knowledge
    ):
        for table_idx in range(move_index, most_recent_move):
            ...
            for card, new_k in cards_affected_with_new_knowledge:
                if card in self.move_tracking_table.at[table_idx, "card ids"]:
                    ...
                    self.move_tracking_table.at[table_idx, "hand knowledge"][
                        pos_idx
                    ] = new_k
            ...
            sample_conditionals = [
                sample.conditional_probs
                for sample in self.move_tracking_table.loc[table_idx][
                    "generated samples"
                ]
            ]
            new_conditional = np.mean(np.array(sample_conditionals), axis=0)
            self.move_tracking_table.at[
                table_idx, "conditional probabilities"
            ] = new_conditional
            ...
            if prev_idx >= 0:
                prev_prior = self.move_tracking_table.loc[prev_idx][
                    "agent distribution"
                ]
                prev_prior2 = self.move_tracking_table.loc[prev_idx][
                    "MLE probabilities"
                ]
            else:
            ...
            self.move_tracking_table.at[table_idx, "MLE probabilities"] = (
                new_conditional + prev_prior2 * table_idx
            ) / (table_idx + 1)
