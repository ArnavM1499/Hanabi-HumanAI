...
class Net(torch.nn.Module):
    ...
    def forward(self, padded, lengths):
        ...
        max_length, _, _ = padded.shape
        if self.window < 0 or max_length <= self.window:
            packed_output, _ = self.lstm(packed_output)
            padded_output, _ = torch.nn.utils.rnn.pad_packed_sequence(packed_output)
        else:
            inputs = [
                torch.nn.utils.rnn.pack_padded_sequence(
                    padded_output[i : min(self.window + i, max_length)],
                    [min(max(1, x - i), self.window) for x in lengths],
                )
                for i in range(max_length - self.window)
            ]
            outputs = []
            for i, sequence in enumerate(inputs):
                out, _ = self.lstm(sequence)
                out, _ = torch.nn.utils.rnn.pad_packed_sequence(packed_output)
                if i == 0:
                    outputs.append(out)
                else:
                    outputs.append(out[-1:])
            padded_output = torch.cat(outputs)
        padded_output = self.output_fc(padded_output)
        packed_output = torch.nn.utils.rnn.pack_padded_sequence(padded_output, lengths)
        return packed_output


model = Net([512], 512, 2, [], drop_out=True, window=WINDOW).to(DEVICE)
